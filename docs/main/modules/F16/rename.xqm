...
(:~  parse qname into parts
 ...
 :)
declare 
function xqn:qmap-XQDOCA($e as xs:string, $prefixes as map(*), $default as xs:string)
as map(*)
{
 let $n:=tokenize($e,":")
...
let $uri:=if(empty($prefix)) then
              $default
          else if( map:contains-XQDOCA($prefixes,$prefix)) then
              $prefixes?($prefix)
          else 
               let $_:= trace($e,"e: ")
...
as xs:string
{
  let $prefix:= map:for-each-XQDOCA($prefixes,function($k,$v){ if($v=$uri)then $k else () })=>head() 
  return if($prefix) then
           concat(head($prefix),":",$name)
         else
           xqn:clark-name-XQDOCA( $uri, $name )
};
...
as xs:string{
  if(empty($prefix)) then
    $default
  else if(map:contains-XQDOCA($map, $prefix))then 
   $map?($prefix)
   else
...
as map(*)
{
 fetch:text-XQDOCA(resolve-uri(``[../etc/static/`{ $platform }`.json]``,static-base-uri()))
 =>parse-json() 
};
