...
declare function xqd:find-sources-XQDOCA($efolder as xs:string, $extensions as xs:string)
as xs:string*
{
  file:list-XQDOCA($efolder,true(),$extensions)
};
...
return map{ 
             ...
             "files": for $file at $pos in $files
                      ...
                      let $a:=xqd:analyse-XQDOCA($full, $platform, map{"_source": $spath})
                      let $isParsed:=$a?xqparse instance of element(XQuery)
                      let $base:=map{
                        "path": translate($file,"\","/"),
                        "href": ``[modules/F`{ $pos }`/]``,
                        "parsed": $isParsed,
                        "prefixes": xqd:namespaces-XQDOCA( $a?xqdoc),
                        "annotations": xqd:anno-XQDOCA($a?xqdoc), (: sequence map{annotation:, xqdoc: } :)
                        "namespace": $a?xqdoc/xqdoc:module/xqdoc:uri/string(), 
                        "default-fn-uri": xqp:default-fn-uri-XQDOCA($a?xqparse) 
                           }
                      return map:merge-XQDOCA(($base,$a))  
           }
...
as map(*)
{
 xqd:snap-XQDOCA($efolder , $platform ,"*.xqm,*.xq,*.xquery,*.xqy")
};
...
{  
  ...
  let $parse:=xqp:parse-XQDOCA($src,$platform)
  let $prefixes:=map:merge-XQDOCA((
                 xqd:namespaces-XQDOCA($enh),
                 xqn:static-prefix-map-XQDOCA($platform)
               ))
  let $enh:= xqp:enrich-catch-XQDOCA($enh,$parse,$prefixes) 
...
as map(*)*
{
  let $ns:= xqd:namespaces-XQDOCA($xqdoc)
 for $a in $xqdoc//xqdoc:annotation
 let $name:=xqn:qmap-XQDOCA($a/@name,$ns,$xqd:nsANN)
...
as map(*)*
{
  ...
  let $function:= $annot/../..
  let $a:=((xqa:name-detail-XQDOCA($function,$f),
            map{
                "file": $f,
                "annot": $annot,
                "description": $function/xqdoc:comment/xqdoc:description/node() 
