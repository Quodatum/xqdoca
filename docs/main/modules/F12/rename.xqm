...
declare variable $xqp:xparse_opts:=map{
 ...
 :)
declare function xqp:enrich-catch-XQDOCA($xqdoc as element(xqdoc:xqdoc),
                                  $xqparse as element(*),
                                  $prefixes)
as element(xqdoc:xqdoc)
{
try{
      xqp:enrich-XQDOCA($xqdoc,$xqparse,$prefixes) 
    }   catch * { 
...
as element(xqdoc:xqdoc)
{
  ...
  let $xqdoc:= $xqdoc transform with {
                       ...
                       if ($body) then (
                         insert node xqp:main-XQDOCA($body) as last into xqdoc:functions,
  ...
  (: swap imports and namespaces basex err :) 
   let $xqdoc:= $xqdoc transform with {
    ...
    let $def-fn:= xqp:default-fn-uri-XQDOCA($xqparse)
                   
     let $fmap:=map:merge-XQDOCA((
                xqp:funmap-XQDOCA($xqparse, $prefixes, $def-fn),
                if($body) then map:entry-XQDOCA("Q{http://www.w3.org/2005/xquery-local-functions}xqDoc-main#0",$body) else ()
         ))                       
   (: insert function source :)
  let $xqdoc:= $xqdoc transform with {
    for $f in ./xqdoc:functions/xqdoc:function
  
    let $name:=xqn:qmap-XQDOCA($f/xqdoc:name,$prefixes, $def-fn)
    let $key:=concat("Q{",$name?uri,"}",$name?name,"#",$f/@arity)
    let $parse:= map:get-XQDOCA($fmap,$key)
    return if(map:contains-XQDOCA($fmap,$key))then
                   (   
                    insert node xqp:references-XQDOCA($parse,$prefixes, $def-fn) into $f,
                    insert node <xqdoc:body>{$parse/string()}</xqdoc:body> into $f
                  )
               else
                  let $a:=trace(map:keys-XQDOCA($fmap),"FMAP: ")
                  return error(xs:QName-XQDOCA("xqp:enrich"),"key not found " || $key)  
  }
...
as element(*)*
{
  $e//FunctionCall!xqp:invoke-fn-XQDOCA(.,$prefixes, $def-fn),
  $e//ArrowExpr!xqp:invoke-arrow-XQDOCA(.,$prefixes, $def-fn),
  $e//VarRef!xqp:ref-variable-XQDOCA(.,$prefixes, $def-fn) 
};
...
let $qname:=xqn:qmap-XQDOCA($fname,$prefixes, $def-fn)
...
(
  ...
  !map:entry-XQDOCA(NCName[1]/string(),StringLiteral[1]/substring(.,2,string-length(.)-2))
)=>map:merge()  
};
...
as map(*)
{(
  xqp:namespaces-XQDOCA($e),
 xqn:static-prefix-map-XQDOCA($platform)
) =>map:merge()
};
 ...
 let $items:=for $f in $e//FunctionDecl
             ...
             let $name:=if($name instance of element(QName)
                        or $name instance of element(TOKEN)) then
                              xqn:qmap-XQDOCA($name,$prefixes,$def-fn)
                        else if($name instance of element(URIQualifiedName)) then
                                xqn:uriqname-XQDOCA($name)
                        else 
                             let $_:=trace($name,"name")
                             return error(xs:QName-XQDOCA("xqp:funmap"), "bad name: ", $name)
             let $arity:=count($f/(Param|ParamList/Param))
             let $key:=concat("Q{",$name?uri,"}",$name?name,"#",$arity)
             return map:entry-XQDOCA($key,$f)
 return map:merge-XQDOCA($items)
};
...
as map(*)
{
  ...
  !map:entry-XQDOCA(string(@prefix),string(@uri))
  =>map:merge()
};
...
as element(*)
{  
  xp:parse-XQDOCA($xq || "",$xqp:xparse_opts($platform)) 
};
