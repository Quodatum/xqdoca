...
fn:fold-right-XQDOCA($a,
             (),
             function($this,$acc){ tree:merge-XQDOCA($acc,tree:nest-XQDOCA($this,$delimiter)) }
            )
}; 
...
as element(*)
{
  ...
  let $parts:=fn:tokenize-XQDOCA(($path),$delimiter)
...
as xs:string{
  ...
  let $tail:= if ($ok) then  tree:base-XQDOCA($c) else ()
