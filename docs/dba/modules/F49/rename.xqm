...
function dba:user-XQDOCA(
  ...
  let $user := user:list-details-XQDOCA($name)
  let $admin := $name eq 'admin'
  return html:wrap-XQDOCA(map { 'header': ($dba:CAT, $name), 'info': $info, 'error': $error },
    <tr>
      <td width='49%'>
        <form action="user-edit" method="post" autocomplete="off">
          ...
          <h2>{
            html:link-XQDOCA('Users', $dba:CAT), ' » ',
            $name, ' » ',
            html:button-XQDOCA('save', 'Save')
          }</h2>
          ...
          <form action="{ $dba:SUB }" method="post" id="{ $dba:SUB }" class="update">
            ...
            <div class='small'/>
            {
              ...
              let $buttons := if($admin) then () else (
                html:button-XQDOCA('pattern-add', 'Add…'),
                html:button-XQDOCA('pattern-drop', 'Drop', true())
              )
              return html:table-XQDOCA($headers, $entries, $buttons, map { }, map { })
            }
          </form>
          <div class='note'>
...
function dba:user-redirect(
  ...
  web:redirect-XQDOCA($action, map { 'name': $name, 'pattern': $patterns })
};
