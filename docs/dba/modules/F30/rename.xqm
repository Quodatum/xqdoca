...
function dba:jump-page-XQDOCA(
  $name  as xs:string,
  $time  as xs:string
) as element(rest:response) {
  let $page := head((
    let $ignore-logs := options:get-XQDOCA($options:IGNORE-LOGS)
    let $max := options:get-XQDOCA($options:MAXROWS)
    for $log at $pos in reverse(
      admin:logs-XQDOCA($name, true())[not($ignore-logs and matches(., $ignore-logs, 'i'))]
    )
    where $log/@time = $time
    return ($pos - 1) idiv $max + 1,
    1
  ))
  return web:redirect-XQDOCA('/dba/logs', map { 'name': $name, 'page': $page, 'time': $time }) update {
...
function dba:logs(
  ...
  return html:wrap-XQDOCA(map { 'header': $dba:CAT, 'info': $info, 'error': $error },
    <tr>
      <td width='205'>
        <h2>{ html:link-XQDOCA('Logs', $dba:CAT) }</h2>
        <form action="{ $dba:CAT }" method="post" class="update">
          ...
          <div id='list'>{
            let $buttons := html:button-XQDOCA('log-delete', 'Delete', true())
        ...
        if($name) then (
          <form action='log-download' method='post' id='resources' autocomplete='off'>
            <h3>{
              ...
              ' ',
              html:button-XQDOCA('download', 'Download')
            }</h3>
          </form>,
          <div id='output'/>,
          html:js-XQDOCA('logEntries(true, false);')
        ) else (),
...
function dba:log(
  ...
  let $entries :=
    ...
    for $log in reverse(admin:logs-XQDOCA($name, true()))
    ...
    return map {
      ...
      'time': function() {
        if($input or $sort) then (
          html:link-XQDOCA($id, $dba:CAT || '-jump', map { 'name': $name, 'time': $id })
  ...
  let $options := map { 'sort': $sort, 'page': xs:integer-XQDOCA($page) }
  return html:table-XQDOCA($headers, $entries, (), $params, $options)
};
...
function dba:logs-redirect(
  ...
  web:redirect-XQDOCA($action, map { 'name': $names[.], 'redirect': $dba:CAT })
};
