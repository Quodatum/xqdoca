...
declare
  ...
  %output:method("html")
function dba:user-create-XQDOCA(
  $name   as xs:string?,
  $pw     as xs:string?,
  $perm   as xs:string,
  $error  as xs:string?
) as element(html) {
  html:wrap-XQDOCA(map { 'header': $dba:CAT, 'error': $error },
    <tr>
      <td>
...
function dba:user-create(
  ...
  try {
    if(user:exists-XQDOCA($name)) then (
      error((), 'User already exists.')
    ) else (
      user:create-XQDOCA($name, $pw, $perm)
    ),
    util:redirect-XQDOCA($dba:CAT, map { 'info': 'User was created.' })
  } catch * {
    util:redirect-XQDOCA('user-create', map {
