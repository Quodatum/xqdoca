...
declare
  ...
  %output:method("html")
function dba:pattern-add-XQDOCA(
  $name     as xs:string,
      ...
      <td>
        <form action="pattern-add" method="post" autocomplete="off">
          <h2>{
            ...
            html:button-XQDOCA('create', 'Add Pattern')
          }</h2>
...
function dba:create(
  ...
  try {
    user:grant-XQDOCA($name, $perm, $pattern),
    util:redirect-XQDOCA($dba:SUB, map { 'name': $name, 'info': 'Pattern was created.' })
  } catch * {
    util:redirect-XQDOCA('pattern-add', map {
