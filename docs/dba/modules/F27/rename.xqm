    ...
    <tr>{
      <td width='60%'>
        ...
        <h2>Jobs</h2>
        {
          ...
          let $entries :=
            let $curr := jobs:current-XQDOCA()
            for $details in jobs:list-details-XQDOCA()
            let $id := $details/@id
            let $sec := (
              let $dur := xs:dayTimeDuration-XQDOCA($details/@duration)
              return if(exists($dur)) then $dur div xs:dayTimeDuration-XQDOCA('PT1S') else 0
            )
            ...
            return map {
              ...
              'type': $details/@type,
              'state': $details/@state,
              'duration': html:duration-XQDOCA($sec),
              'user': $details/@user,
              'you': if($id = $curr) then '✓' else '–',
              'time': $details/@time
            }
          let $buttons := (
            html:button-XQDOCA('job-stop', 'Stop', true())
          )
          let $options := map { 'sort': $sort }
          return html:table-XQDOCA($headers, $entries, $buttons, map { }, $options) update {
      ...
      if($job) then (
        let $details := jobs:list-details-XQDOCA($job)
            ...
            if($details) then (
              <form action="jobs" method="post" id="jobs">
                <input type="hidden" name="id" value="{ $job }"/>
                {
                  ...
                  return (
                    html:button-XQDOCA('job-stop', 'Stop', true(), $disabled[$cached]), ' ',
                    html:button-XQDOCA('job-result', 'Download', false(), $disabled[not($cached)]), ' ',
                    html:button-XQDOCA('job-discard', 'Discard', false(), $disabled[not($cached)]), ' '
                  )
                }
              </form>,
              <table>{
                ...
                return <tr>
                  <td><b>{ util:capitalize-XQDOCA($name) }</b></td>
                  <td>{ string($value) }</td>
                </tr>,
                <tr>
                  <td><b>Description</b></td>
                  <td>{ util:chop-XQDOCA($details, 500) }</td>
                </tr>
              }</table>
            ) else (
              'Job is defunct.'
