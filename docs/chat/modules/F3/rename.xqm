...
declare
  %rest:path('/chat')
  %output:method('html')
function chat:chat-XQDOCA() as element() {
  if(session:get-XQDOCA($chat-util:id)) then (
    chat:main-XQDOCA()
  ) else (
    chat:login-XQDOCA()
  )
...
function chat:login-check(
  ...
  try {
    ...
    session:set-XQDOCA($chat-util:id, $name)
  } catch user:* {
    (: login fails: no session info is set :)
  },
  web:redirect-XQDOCA('/chat')
};
...
function chat:logout() as element(rest:response) {
  session:get-XQDOCA($chat-util:id) ! chat-util:close-XQDOCA(.),
  session:delete-XQDOCA($chat-util:id),
  web:redirect-XQDOCA('/chat')
};
...
declare %private function chat:wrap(
  ...
  <html>
    ...
    <body>
      <span class='right'>
        {
          for $id in session:get-XQDOCA($chat-util:id)
