...
declare %a:since("basex", "7.5") %a:until("basex", "8.0", "user:list-details#1") function admin:users() as element(user)* external;
declare %a:since("basex", "7.5") %a:until("basex", "8.0", "user:list-details#2") function admin:users($db as xs:string) as element(user)* external;
